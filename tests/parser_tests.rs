...
fn test_hash_generation() {
    ...
    assert_ne!(parsed1.hash, parsed2.hash, "Different code should have different hashes");

    // Same code should have same hash
    let parsed1_again = parser.parse_file(path, code1).unwrap();
    assert_eq!(parsed1.hash, parsed1_again.hash, "Same code should have same hash");
}
...
"#;
    ...
    assert!(!parsed.function_calls.is_empty(), "Should find function calls");

    // The main function should call helper and another_helper
    let helper_calls: Vec<_> = parsed.function_calls.iter()
        .filter(|c| c.callee_name == "helper" || c.callee_name == "another_helper")
        .collect();

    assert!(helper_calls.len() >= 2, "Should find calls to helper and another_helper");
}
...
"#;
    ...
    let method_calls: Vec<_> = parsed.function_calls.iter()
        .filter(|c| c.callee_name == "increment" || c.callee_name == "get")
        .collect();

    assert!(method_calls.len() >= 2, "Should find method calls to increment and get");
}
